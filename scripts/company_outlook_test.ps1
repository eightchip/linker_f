# 会社PC用 Outlook接続テストスクリプト
# 構文エラーを修正した安全版

Write-Host "=== 会社PC用 Outlook接続テスト ===" -ForegroundColor Cyan
Write-Host ""

# 1. PowerShell実行ポリシー確認
Write-Host "1. PowerShell実行ポリシー確認:" -ForegroundColor Yellow
$executionPolicy = Get-ExecutionPolicy
Write-Host "   現在の実行ポリシー: $executionPolicy" -ForegroundColor White
if ($executionPolicy -eq "Restricted") {
    Write-Host "   警告: 実行ポリシーが制限されています" -ForegroundColor Yellow
...
if ($outlookProcesses) {
    ...
    Write-Host "   Outlook が起動していません" -ForegroundColor Yellow
...
try {
    ...
    Write-Host "   COM オブジェクトの作成に成功しました" -ForegroundColor Green
...
try {
    ...
    if ($accounts.Count -gt 0) {
        ...
        Write-Host "   メールアカウントが設定されていません" -ForegroundColor Yellow
...
try {
    ...
    if ($mailCount -gt 0) {
        ...
        Write-Host "   送信済みメールがありません" -ForegroundColor Yellow
